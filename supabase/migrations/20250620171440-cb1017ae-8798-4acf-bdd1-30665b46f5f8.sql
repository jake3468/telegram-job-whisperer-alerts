...
-- Drop the existing function to recreate it with proper configuration
DROP FUNCTION IF EXISTS public.handle_company_role_analysis_webhook();

-- Recreate the trigger function with enhanced logging and error handling
...
BEGIN
    ...
    RAISE LOG 'Company role analysis webhook trigger started for record %', NEW.id;
    ...
    IF user_data.id IS NULL OR user_profile_data.id IS NULL THEN
        RAISE LOG 'Missing user or profile data for company-role analysis %. User data: %, Profile data: %', NEW.id, user_data.id, user_profile_data.id;
        RETURN NEW;
    END IF;
    
    RAISE LOG 'Found user data for company analysis %. User ID: %, Profile ID: %', NEW.id, user_data.id, user_profile_data.id;
    ...
    IF n8n_webhook_url IS NULL THEN
        RAISE LOG 'N8N_COMPANY_WEBHOOK_URL secret not found for company analysis %', NEW.id;
        RETURN NEW;
    END IF;
    ...
    payload := jsonb_build_object(
        ...
        'anti_duplicate_metadata', jsonb_build_object(
            ...
            'trigger_source', 'company_analysis_trigger_v2',
            'submission_time', extract(epoch from now())
        )
    );
    
    RAISE LOG 'Calling edge function for company analysis % with payload size %', NEW.id, length(payload::text);
    ...
    PERFORM net.http_post(
        ...
        headers := jsonb_build_object(
            ...
            'X-Source', 'company-analysis-trigger-v2',
            'X-Webhook-Type', 'company_analysis'
        ),
        body := payload
    );
    
    RAISE LOG 'Company-role analysis webhook triggered successfully for record % with fingerprint %', NEW.id, fingerprint;
    
    RETURN NEW;
EXCEPTION
    WHEN OTHERS THEN
        RAISE LOG 'Error in company-role analysis webhook for record %: %', NEW.id, SQLERRM;
        RETURN NEW;
END;
