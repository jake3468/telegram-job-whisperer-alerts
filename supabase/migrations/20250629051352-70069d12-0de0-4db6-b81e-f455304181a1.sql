...
-- Update the process_payment_credits function to handle subscription.active events
...
BEGIN
  ...
  -- Process based on event type and status - UPDATED to include subscription.active
  IF p_event_type IN ('payment.completed', 'subscription.renewed', 'subscription.created', 'subscription.active') AND p_status = 'active' THEN
