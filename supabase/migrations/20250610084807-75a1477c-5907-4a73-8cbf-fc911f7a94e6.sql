...
DROP TRIGGER IF EXISTS job_analysis_webhook_trigger ON public.job_analyses CASCADE;
DROP TRIGGER IF EXISTS trigger_job_analysis_webhook ON public.job_analyses CASCADE;
DROP TRIGGER IF EXISTS trigger_cover_letter_webhook ON public.job_cover_letters CASCADE;
DROP TRIGGER IF EXISTS trigger_cover_letter_webhook_secure ON public.job_cover_letters CASCADE;

-- Now drop all the functions
DROP FUNCTION IF EXISTS public.call_job_analysis_webhook() CASCADE;
DROP FUNCTION IF EXISTS public.handle_cover_letter_webhook() CASCADE;
DROP FUNCTION IF EXISTS public.handle_cover_letter_webhook_secure() CASCADE;

-- Create a minimal trigger function that only handles the basic webhook call
...
BEGIN
    ...
    IF user_data.id IS NULL OR user_profile_data.id IS NULL THEN
        RAISE LOG 'Missing user or profile data for cover letter %', NEW.id;
        RETURN NEW;
    END IF;
    ...
    -- Build payload - explicitly separate user and user_profile data
    payload := jsonb_build_object(
        ...
        'anti_duplicate_metadata', jsonb_build_object(
            ...
            'trigger_source', 'cover_letter_trigger_v4',
    ...
    PERFORM net.http_post(
        ...
        headers := jsonb_build_object(
            ...
            'X-Source', 'cover-letter-trigger-v4',
