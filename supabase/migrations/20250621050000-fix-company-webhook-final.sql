...
-- Create the webhook trigger function with enhanced logging and error handling
...
BEGIN
    -- Only trigger on INSERT operations (when user clicks Generate Analysis)
    ...
    IF n8n_webhook_url IS NULL OR n8n_webhook_url = '' THEN
        RAISE LOG '[COMPANY_WEBHOOK] ERROR: N8N_COMPANY_WEBHOOK_URL secret not found or empty for company analysis: %', NEW.id;
        RETURN NEW;
    END IF;
    ...
    payload := jsonb_build_object(
        ...
        'anti_duplicate_metadata', jsonb_build_object(
            ...
            'trigger_source', 'company_analysis_trigger_final',
    ...
    -- Make the HTTP POST request to the edge function with timeout
    BEGIN
        SELECT * INTO http_response FROM net.http_post(
            ...
            headers := jsonb_build_object(
                ...
                'X-Source', 'company-analysis-trigger-final',
...
DO $$
BEGIN
    RAISE LOG '[COMPANY_WEBHOOK] Migration completed successfully - trigger and function recreated';
END $$;
