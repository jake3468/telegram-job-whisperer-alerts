...
-- Drop any existing trigger and function for interview prep
DROP TRIGGER IF EXISTS trigger_interview_prep_webhook ON public.interview_prep;
DROP FUNCTION IF EXISTS public.handle_interview_prep_webhook();

-- Create trigger function to handle interview prep webhook
...
-- Create the trigger on the interview_prep table
...
-- Enable realtime for interview_prep table so we can listen for updates
