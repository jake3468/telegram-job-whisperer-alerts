...
UPDATE public.job_alerts 
SET job_type = CASE 
  WHEN job_type::text = 'Remote' THEN 'full-time'
  WHEN job_type::text = 'On-site' THEN 'full-time'
  WHEN job_type::text = 'Hybrid' THEN 'full-time'
  ELSE 'full-time'
END::text::public.job_type;

-- Create new enum type
DROP TYPE IF EXISTS public.job_type_new;
CREATE TYPE public.job_type_new AS ENUM ('full-time', 'part-time', 'contract', 'intern');

-- Update column to use new enum
ALTER TABLE public.job_alerts ALTER COLUMN job_type TYPE public.job_type_new USING 'full-time'::public.job_type_new;
