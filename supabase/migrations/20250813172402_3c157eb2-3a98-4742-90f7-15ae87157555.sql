...
-- Create a more robust clerk ID validation function
CREATE OR REPLACE FUNCTION public.get_current_clerk_user_id_ultra_secure()
RETURNS TEXT
LANGUAGE SQL
...
-- Update policies to use the ultra-secure function
...
USING (
  clerk_id IS NOT NULL 
  AND clerk_id = get_current_clerk_user_id_ultra_secure()
  AND auth.uid() IS NOT NULL
