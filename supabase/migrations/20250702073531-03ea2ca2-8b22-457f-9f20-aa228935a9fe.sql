-- Update the process_payment_credits function to handle the correct event types and statuses
...
BEGIN
  ...
  -- Process based on event type and status - UPDATED to handle payment.succeeded, subscription.active, subscription.renewed
  IF p_event_type IN ('payment.succeeded', 'subscription.active', 'subscription.renewed') AND p_status IN ('succeeded', 'active') THEN
    ...
    IF credits_to_award > 0 THEN
      ...
      ELSE
        transaction_type_value := 'credit_pack_purchase'; -- fallback
      END IF;
