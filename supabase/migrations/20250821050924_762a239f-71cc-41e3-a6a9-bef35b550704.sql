...
-- Remove overly permissive service role policy and ensure proper user access control
...
-- Ensure the user_id column is not nullable to prevent security issues
-- (We'll make this change only if it doesn't break existing data)
UPDATE public.add_job_chat_history_new 
SET user_id = (SELECT id FROM users WHERE clerk_id = 'system' LIMIT 1)
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'fix_applied', 'restricted_rls_policies',
    'timestamp', now()
  )
);