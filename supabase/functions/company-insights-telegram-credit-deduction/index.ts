...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Processing Telegram company insights credit deduction for profile: ${user_profile_id}`)
    ...
    const { data: deductionResult, error: deductionError } = await supabase
      .rpc('deduct_credits', {
        ...
        p_feature_used: 'company_insights_telegram',
        p_description: 'Company insights - telegram'
      })
    ...
    const { data: transactionData, error: transactionError } = await supabase
      ...
      .select('*')
      .eq('user_id', userId)
      .eq('feature_used', 'company_insights_telegram')
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: 'Company insights credits deducted successfully',
    ...
    console.error('Unexpected error in company insights credit deduction:', error)
    return new Response(
      JSON.stringify({ 
