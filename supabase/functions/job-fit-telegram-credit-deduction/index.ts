...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Processing Telegram job fit credit deduction for profile: ${user_profile_id}`)
    ...
    const { data: deductionResult, error: deductionError } = await supabase
      .rpc('deduct_credits', {
        ...
        p_feature_used: 'job_fit_telegram',
        p_description: 'Job Fit - telegram'
      })
    ...
    const { data: transactionData, error: transactionError } = await supabase
      ...
      .select('*')
      .eq('user_id', userId)
      .eq('feature_used', 'job_fit_telegram')
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: 'Job Fit credits deducted successfully',
    ...
    console.error('Unexpected error in job fit credit deduction:', error)
    return new Response(
      JSON.stringify({ 
