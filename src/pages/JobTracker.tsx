...
const JobTracker = () => {
  ...
  const columns = [
    { key: 'saved', title: 'Saved', canAdd: true, color: 'bg-slate-700' },
    { key: 'applied', title: 'Applied', canAdd: true, color: 'bg-blue-700' },
    { key: 'interview', title: 'Interview', canAdd: true, color: 'bg-orange-700' },
    { key: 'rejected', title: 'Rejected', canAdd: false, color: 'bg-red-700' },
    { key: 'offer', title: 'Offer', canAdd: false, color: 'bg-green-700' }
  ];
  ...
  const handleAddJob = async () => {
    ...
    try {
      ...
      const maxOrder = Math.max(
        ...jobs.filter(job => job.status === selectedStatus).map(job => job.order_position),
        -1
      );
      ...
      setFormData({ company_name: '', job_title: '', job_description: '', job_url: '' });
  ...
  const moveJob = async (jobId: string, direction: 'left' | 'right') => {
    ...
    const maxOrder = Math.max(
      ...jobs.filter(j => j.status === newStatus).map(j => j.order_position),
      -1
    );
    ...
    <Layout>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 max-w-7xl mx-auto">
        {columns.map((column, columnIndex) => (
          <div key={column.key} className="bg-gray-800/50 rounded-lg p-4 min-h-[500px]">
                  ...
                  <DialogContent className="bg-gray-900 border border-gray-700 text-white">
                    ...
                    <div className="space-y-4">
                      ...
                      <Button 
                        ...
                        className="w-full bg-blue-600 hover:bg-blue-700 text-white font-orbitron"
                      >
                        Add Job
                      </Button>
                    </div>
            ...
            <div className="space-y-3">
              {getJobsByStatus(column.key).map((job) => (
                <div
                  key={job.id}
                  className="bg-gray-700/80 rounded-lg p-3 border border-gray-600 hover:border-gray-500 transition-all group"
                >
                  ...
                  {job.job_description && (
                    <p className="text-gray-400 text-xs mb-2 line-clamp-2">{job.job_description}</p>
                  )}
