...
const LinkedInPosts = () => {
  ...
  // Deferred credit deduction when all LinkedIn posts are ready and displayed
  useEffect(() => {
    if (postsData && areAllPostsReady(postsData) && !isGenerating && !hasCreditBeenDeducted && currentPostId) {
      const deductCreditsForLinkedInPosts = async () => {
        const success = await deductCredits(1.5, 'linkedin_posts', 'LinkedIn Posts - 3 professional post variations');
        if (success) {
          setHasCreditBeenDeducted(true);
          console.log('Successfully deducted 1.5 credits for LinkedIn posts display');
        }
      };
      
      deductCreditsForLinkedInPosts();
    }
  }, [postsData, isGenerating, hasCreditBeenDeducted, currentPostId, deductCredits]);
        ...
        <div className="flex items-center justify-between p-3">
          ...
          <div className="flex items-center gap-2">
            <img
              ...
              className="max-h-8 drop-shadow-2xl object-fill"
            />
            <span className="font-orbitron drop-shadow bg-gradient-to-r from-sky-400 via-fuchsia-400 to-indigo-400 bg-clip-text text-white font-bold min-w-0 truncate text-lg">
              Aspirely.ai
            </span>
          </div>
        </div>
            ...
            <div className="min-h-screen bg-gradient-to-br from-black via-slate-900 to-slate-800 pt-0 flex flex-col">
              <div className="container mx-auto px-2 sm:px-4 py-8 bg-transparent rounded-3xl mt-4 mb-8 max-w-6xl w-full min-w-0">
                ...
                <div className="max-w-5xl mx-auto min-w-0">
                      ...
                      <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <SelectContent className="bg-black/80 border-teal-200/30 text-white">
                              {toneOptions.map((option) => (
                                <SelectItem
                                  key={option.value}
                                  value={option.value}
                                  className="font-medium data-[highlighted]:bg-teal-200 data-[highlighted]:text-black"
                                >
                                  {option.label}
                                </SelectItem>
                              ))}
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 pt-4">
                          <Button
                            ...
                            className="flex-1 bg-gradient-to-r from-indigo-500 via-purple-500 to-blue-500 hover:from-indigo-600 hover:via-purple-600 hover:to-blue-600 text-white font-semibold text-base h-12 shadow-md rounded-lg"
                          >
                  ...
                  {/* Loading State for Post Generation - Show immediately when isGenerating is true */}
                  {isGenerating && (
