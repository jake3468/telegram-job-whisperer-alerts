...
const LinkedInPosts = () => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        if (payload.eventType === 'UPDATE' && payload.new) {
          ...
          if (areAllPostsReady(newData) && !successToastShown) {
  ...
  }, [currentPostId, isAuthReady, toast, deductCreditsAfterResults, successToastShown]);

  // Check existing data when currentPostId changes
  useEffect(() => {
    const checkExistingData = async () => {
      if (!currentPostId || !isAuthReady || successToastShown) return;
    ...
    if (currentPostId && isGenerating && !successToastShown) {
      checkExistingData();
    }
  }, [currentPostId, isAuthReady, isGenerating, executeWithRetry, toast, deductCreditsAfterResults, successToastShown]);
