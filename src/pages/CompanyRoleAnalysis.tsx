        ...
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="text-center space-y-4">
            <div className="flex items-center justify-center gap-3 mb-4">
              ...
              <h1 className="text-4xl sm:text-5xl font-orbitron font-extrabold text-transparent bg-gradient-to-r from-violet-400 via-purple-500 to-indigo-400 bg-clip-text lg:text-4xl">Company Decoder</h1>
            </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input id="companyName" type="text" placeholder="e.g., Google, Microsoft, Amazon" value={companyName} onChange={e => setCompanyName(e.target.value)} className="bg-violet-950/50 border-violet-500/30 text-violet-100 placeholder:text-violet-400 focus:border-violet-400 focus:ring-violet-400/20" required />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="location" type="text" placeholder="e.g., San Francisco, New York, Remote" value={location} onChange={e => setLocation(e.target.value)} className="bg-violet-950/50 border-violet-500/30 text-violet-100 placeholder:text-violet-400 focus:border-violet-400 focus:ring-violet-400/20" required />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input id="jobTitle" type="text" placeholder="e.g., Senior Software Engineer, Product Manager, Data Scientist" value={jobTitle} onChange={e => setJobTitle(e.target.value)} className="bg-violet-950/50 border-violet-500/30 text-violet-100 placeholder:text-violet-400 focus:border-violet-400 focus:ring-violet-400/20" required />
                </div>

                {/* Action Buttons */}
                <div className="flex flex-col sm:flex-row gap-4 pt-4">
                  <Button type="submit" disabled={isSubmitting || !hasCredits} className="flex-1 bg-gradient-to-r from-violet-600 via-purple-600 to-indigo-600 hover:from-violet-700 hover:via-purple-700 hover:to-indigo-700 text-white font-orbitron font-bold py-6 text-lg shadow-2xl shadow-violet-600/25 border-0">
                    {isSubmitting ? <>
                  ...
                  <Button type="button" variant="outline" onClick={handleReset} className="border-violet-500/50 text-violet-300 hover:bg-violet-800/20 hover:text-violet-200 font-orbitron">
          ...
          {analysisHistory && analysisHistory.length > 0 && <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Button onClick={() => setIsHistoryOpen(true)} variant="outline" className="border-violet-500/50 text-violet-300 hover:bg-violet-800/20 hover:text-violet-200 font-orbitron">
                  View All History
                </Button>
              </div>
              
              <div className="grid gap-4">
                {analysisHistory.slice(0, 3).map(analysis => <Card key={analysis.id} className="bg-black/40 border-violet-500/20 backdrop-blur-sm">
                    <CardContent className="p-4">
                      ...
                      {analysis.analysis_result && <div className="mt-3 p-3 bg-violet-950/30 rounded-lg border border-violet-500/20">
