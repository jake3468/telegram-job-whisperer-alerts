...
const ComparisonSlider = () => {
  ...
  const withoutItems = ["Old job postings", "Hours wasted searching", "Late to apply", "No tracking system", "Same resume for all", "No interview prep", "Missed opportunities"];
  const withItems = ["Apply before others, with all files ready in 1 click", "Only jobs from last 24 hours", "Matches your title and location", "Daily alerts on Telegram", "All applications tracked", "Cover letters, resume, company insights, job fit check instantly", "Visa info for international students", "Interview prep and AI mock interviews on your phone"];
  return <section className="pt-8 pb-16 px-4 bg-black relative overflow-hidden">
      <div className="max-w-6xl mx-auto">
        <div ref={containerRef} className="relative rounded-2xl overflow-hidden shadow-2xl cursor-grab active:cursor-grabbing" style={{
        userSelect: 'none'
      }}>
          {/* Left Side - With Aspirely.ai */}
          <div className="bg-gradient-to-br from-green-950 to-green-900 p-4 md:p-6 font-sans" style={{
          ...
          minHeight: '100%'
            ...
            <p className="text-green-200 mb-4 text-sm md:text-base">Set up in less than 2 minutes and start getting results.</p>
            <div className="space-y-2 pb-4">
              {withItems.map((item, index) => <div key={index} className="flex items-start gap-2">
          ...
          <div className="bg-gradient-to-br from-red-950 to-red-900 p-4 md:p-6 font-sans" style={{
          ...
          minHeight: '100%'
            ...
            <div className="space-y-2 pb-4">
              {withoutItems.map((item, index) => <div key={index} className="flex items-start gap-2">
          ...
          {/* Slider Handle */}
          <div className="absolute top-0 bottom-0 flex items-center z-10 touch-none" style={{
          left: `${sliderPosition}%`,
          ...
          {/* Center Line */}
          <div className="absolute top-0 bottom-0 w-0.5 bg-white/30 pointer-events-none" style={{
          left: `${sliderPosition}%`,
          transform: 'translateX(-50%)'
        }} />
        </div>
