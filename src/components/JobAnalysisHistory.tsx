...
const JobAnalysisHistory = ({
    ...
    try {
      ...
      const {
        ...
        error
          ...
          ${resultField},
          created_at
        `).eq('user_id', profileData.id).not(resultField, 'is', null).order('created_at', {
        ascending: false
      }).limit(10);
      if (error) {
  ...
  if (isOpen) {
    return <Card className={`${gradientColors} ${borderColors} shadow-2xl`}>
        <CardHeader className="pb-3">
          <CardTitle className="text-white font-inter flex items-center gap-2 text-sm">
            ...
            <Button onClick={() => setIsOpen(false)} size="sm" className="ml-auto bg-white/20 hover:bg-white/30 text-white border-white/20 text-xs px-2 py-1">
              Close
            </Button>
          </CardTitle>
          <CardDescription className="text-white/80 font-inter text-xs">
            {historyData.length === 0 ? `No previous ${type === 'job_guide' ? 'job analyses' : 'cover letters'} found.` : `Found ${historyData.length} previous ${type === 'job_guide' ? 'job analyses' : 'cover letters'}.`}
          </CardDescription>
        </CardHeader>
        
        {historyData.length > 0 && <CardContent className="pt-0 max-h-96 overflow-y-auto">
            <div className="space-y-3">
              {historyData.map(item => <Card key={item.id} className="bg-white/10 border-white/20">
                  <CardContent className="p-3 bg-gray-800">
                    <div className="space-y-2">
                      ...
                      <Button onClick={() => handleUseHistory(item)} size="sm" className="w-full bg-white/20 hover:bg-white/30 text-white border-white/20 text-xs">
  ...
  return <Button onClick={fetchHistory} disabled={isLoading} className="w-full bg-white/20 hover:bg-white/30 text-white border-white/20 text-xs px-4 py-2">
