...
const ProfessionalResumeSection = () => {
    ...
    try {
      await executeWithAuth(
        async () => {
          ...
          const { error } = await updateUserProfile({ resume: base64Data });
          if (error) {
  ...
  if (loading) {
    return (
      <Card className="border-2 border-blue-400 shadow-none">
    ...
    <AuthenticatedComponent className="p-0 rounded-none bg-transparent shadow-none">
      <Card className="rounded-3xl border-2 border-blue-400/80 bg-gradient-to-br from-blue-600/90 via-blue-700/85 to-blue-900/90 shadow-2xl shadow-blue-500/20 transition-all hover:shadow-blue-500/30 backdrop-blur-sm">
        ...
        <CardContent className="space-y-3 pt-0">
          ...
          {userProfile?.resume && (
            <div className="flex items-center gap-2 p-2 bg-green-900/20 border border-green-400/30 rounded-lg">
          ...
          <div className="space-y-2">
            ...
            <label htmlFor="resume-upload">
              <Button
                type="button"
                disabled={uploading || !isReady}
                className="w-full font-inter font-bold text-xs px-4 py-2 h-9 rounded-lg shadow-lg shadow-blue-500/20 focus-visible:ring-2 focus-visible:ring-blue-300 transition-colors text-white bg-emerald-800 hover:bg-emerald-700 disabled:opacity-50 cursor-pointer"
                asChild
              >
