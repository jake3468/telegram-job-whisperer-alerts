...
// Removed dark mode functionality for performance
// import { useTheme } from 'next-themes';

const AuthHeader = () => {
  const navigate = useNavigate();
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  // Removed theme functionality for performance
  ...
  return (
    ...
    <header className="w-full backdrop-blur-md bg-background/30 fixed top-0 left-0 right-0 z-50 shadow-[0_6px_24px_0px_rgba(16,118,238,0.05)]">
      <div className="flex justify-between items-center px-3 sm:px-6 py-4 max-w-7xl mx-auto">
        ...
        <div className="hidden lg:flex items-center space-x-4">
          {/* Theme Toggle Button - Commented out for performance */}
          {/* <button
        ...
        <div className="lg:hidden flex items-center space-x-2">
          {/* Theme Toggle Button for Mobile - Commented out for performance */}
          {/* <button
            onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
            className="text-foreground hover:text-primary transition-colors duration-200 p-2 rounded-lg hover:bg-muted"
            aria-label="Toggle theme"
