...
const PricingSection = () => {
  ...
  // Enhanced token management for logged-in users
  const sessionManager = useEnhancedTokenManagerIntegration();
  const { updateActivity } = useFormTokenKeepAlive(!!user);

  // Activity tracking for user interactions
  const handleButtonClick = (action: string) => {
    if (user) {
      updateActivity(); // Track authenticated user activity
    }
    console.log(`Pricing action: ${action}`);
  };

  const handleSignUpClick = () => {
    handleButtonClick('signup_attempt');
  };

  const handlePricingInteraction = (planType: string) => {
    handleButtonClick(`pricing_interaction_${planType}`);
  };
    ...
    <section className="py-8 md:py-12 px-4 bg-black">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-6">
          <h2 className="text-3xl sm:text-5xl font-extrabold bg-gradient-to-r from-blue-300 via-blue-400 to-indigo-300 bg-clip-text text-transparent mb-2 font-inter">
            Pricing Plans
          </h2>
        ...
        <div className="grid gap-8 lg:gap-8 grid-cols-1 lg:grid-cols-3 items-stretch max-w-6xl mx-auto">
          {/* Free Plan */}
          <Card 
            className={`flex flex-col rounded-2xl shadow-2xl ${planGradientBg.free} transition-transform duration-500 ease-out hover:scale-[1.02] hover:shadow-blue-400/30 min-h-[420px]`}
            onClick={() => handlePricingInteraction('free')}
          >
            <CardHeader className="text-center pb-4 pt-6 px-4">
              <CardTitle className={`text-xl font-bold mb-2 ${planTextColor.free} font-inter`}>
                Free Plan
              </CardTitle>
            ...
            <CardContent className="grow flex flex-col px-4 pb-4">
              ...
              <div className="mt-auto">
                <SignUpButton mode="modal">
                  <Button 
                    className="w-full py-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-xl font-inter text-sm shadow border-0 font-semibold"
          ...
          <Card 
            className={`flex flex-col rounded-2xl shadow-2xl border-0 ${planGradientBg.subscription} relative transition-transform duration-500 ease-out hover:scale-[1.02] hover:shadow-cyan-400/30 min-h-[460px] mt-8 lg:mt-0`}
            onClick={() => handlePricingInteraction('subscription')}
          >
            ...
            <CardHeader className="text-center pb-4 pt-8 px-4">
              <CardTitle className={`text-xl font-bold mb-2 ${planTextColor.subscription} font-inter`}>
                Monthly Subscription
              </CardTitle>
            ...
            <CardContent className="grow flex flex-col px-4 pb-4">
              ...
              <div className="mt-auto">
                <SignUpButton mode="modal">
                  <Button 
                    className="w-full py-2.5 bg-white hover:bg-yellow-100 text-black font-inter text-sm rounded-xl shadow border-0 font-bold transition-colors duration-200" 
                    disabled={isPricingLoading || isProductsLoading}
                    onClick={handleSignUpClick}
                  >
          ...
          {/* Credit Packs */}
          <Card 
            className={`flex flex-col rounded-2xl shadow-2xl ${planGradientBg.pack} transition-transform duration-500 ease-out hover:scale-[1.02] hover:shadow-indigo-400/30 min-h-[420px]`}
            onClick={() => handlePricingInteraction('credit_pack')}
          >
            <CardHeader className="text-center pb-4 pt-6 px-4">
              <CardTitle className={`text-xl font-bold mb-2 ${planTextColor.pack} font-inter`}>
                Credit Packs
              </CardTitle>
              ...
              <div className="flex flex-col gap-2 my-3 flex-grow">
                ...
                {creditPackProducts.length > 0 ? (
                  creditPackProducts.map(pack => (
                    <div 
                      key={pack.product_id} 
                      className="rounded-lg p-2.5 border border-gray-200 flex justify-between items-center shadow hover:shadow-md transition duration-300 bg-blue-100"
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <SignUpButton mode="modal">
                          <Button 
                            size="sm" 
                            className="bg-gray-800 hover:bg-gray-900 text-white text-xs px-3 py-1 h-auto rounded-md"
                            onClick={handleSignUpClick}
                          >
                            Buy
                          </Button>
                  ...
                  !isProductsLoading && pricingData.creditPacks.map(pack => (
                    <div 
                      key={pack.credits} 
                      className="bg-gray-100 rounded-lg p-2.5 border border-gray-200 flex justify-between items-center shadow hover:shadow-md transition duration-300"
                    >
