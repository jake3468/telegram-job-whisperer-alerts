...
export function useOnboardingPopup() {
  ...
  useEffect(() => {
    if (isLoaded && user && userProfile?.show_onboarding_popup) {
      setShowPopup(true);
    }
  }, [isLoaded, user, userProfile?.show_onboarding_popup]);
  ...
  const dontShowAgain = async () => {
    ...
    try {
      const { error } = await supabase
        .from('user_profile')
        .update({ show_onboarding_popup: false })
